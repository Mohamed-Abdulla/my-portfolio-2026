...
import { Code2, Server, Terminal, Smartphone, Activity, Play, ShieldAlert, Cpu } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold font-display">
            I Know that <span className="text-gradient-blue-teal">Good Architecture</span> <br className="hidden sm:inline" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-auto">
          ...
          <div className="md:col-span-2 glass-panel border border-card-border/60 rounded-3xl p-6 sm:p-8 flex flex-col justify-between group relative overflow-hidden">
            <div className="absolute -right-16 -top-16 w-32 h-32 rounded-full bg-primary-accent/5 blur-xl group-hover:opacity-100 transition-opacity duration-500" />
            
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 mb-8">
              <div className="text-left max-w-md">
                <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-card-border w-fit mb-4">
                  <Code2 className="w-6 h-6 text-primary-accent" />
                </div>
                <h3 className="text-xl font-bold font-display text-white mb-2">Backend & API Architecture</h3>
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">
                  Designing secure, high-throughput REST and GraphQL microservices in NestJS and FastAPI. Secured using Kong API Gateway and Keycloak IAM RBAC protocols.
                </p>
              </div>

              {/* Live Interactive API Terminal Mockup */}
              <div className="flex-1 w-full max-w-sm bg-slate-950/80 rounded-2xl border border-card-border/80 overflow-hidden font-mono text-[10px] sm:text-xs text-left shadow-lg">
                ...
                <div className="p-4 space-y-3">
                  <div>
                    <span className="text-accent-teal font-bold">POST</span> <span className="text-slate-300">https://api.colakin.com/v1/auth/verify</span>
                  </div>
                  ...
                  <div className="bg-slate-900/80 border border-card-border p-3 rounded-xl min-h-[85px] relative overflow-hidden flex flex-col justify-center">
                    {apiLoading && (
...
{`{
  ...
  "token_expiry": "3600s"
          ...
          <div className="glass-panel border border-card-border/60 rounded-3xl p-6 sm:p-8 flex flex-col justify-between group relative overflow-hidden text-left">
            <div className="absolute -right-16 -top-16 w-32 h-32 rounded-full bg-violet-400/5 blur-xl group-hover:opacity-100 transition-opacity duration-500" />
            
            <div>
              <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-card-border w-fit mb-4">
                <Server className="w-6 h-6 text-violet-400" />
              </div>
              <h3 className="text-xl font-bold font-display text-white mb-2">Event-Driven Pipelines</h3>
              <p className="text-xs sm:text-sm text-slate-400 leading-relaxed mb-6">
                Orchestrating event queues using Kafka and RabbitMQ to manage high-throughput message streaming workloads asynchronously.
              </p>
            </div>

            {/* Kafka Pipeline Animation mockup */}
            <div className="h-28 bg-slate-950/60 border border-card-border/80 rounded-2xl relative flex items-center justify-between px-4 overflow-hidden">
              ...
              <div className="absolute inset-x-12 h-1 bg-slate-800 rounded-full overflow-hidden">
                <motion.div
                  ...
                  className="w-1/3 h-full bg-gradient-to-r from-transparent via-accent-teal to-transparent"
                />
              </div>
          ...
          <div className="glass-panel border border-card-border/60 rounded-3xl p-6 sm:p-8 flex flex-col justify-between group relative overflow-hidden text-left">
            <div className="absolute -right-16 -top-16 w-32 h-32 rounded-full bg-yellow-400/5 blur-xl group-hover:opacity-100 transition-opacity duration-500" />
            
            <div>
              <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-card-border w-fit mb-4">
                <Cpu className="w-6 h-6 text-yellow-400" />
              </div>
              <h3 className="text-xl font-bold font-display text-white mb-2">DevOps Cluster</h3>
              <p className="text-xs sm:text-sm text-slate-400 leading-relaxed mb-6">
                Automating microservices orchestration on Docker Swarm clusters with Nginx reverse proxy mappings and centralized logging systems.
              </p>
            </div>

            {/* Docker Swarm Cluster Dashboard Visual */}
            <div className="bg-slate-950/60 border border-card-border/80 rounded-2xl p-4 space-y-2.5 font-mono text-[8px] sm:text-[9px]">
              {[
                ...
                <div key={srv.name} className="flex items-center justify-between border-b border-card-border/20 pb-1.5 last:border-0 last:pb-0">
                  <div className="flex items-center gap-1.5">
                    <span className="relative flex h-1.5 w-1.5">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-green-500"></span>
                    </span>
                    <span className="text-slate-300 font-bold">{srv.name}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-accent-teal text-[8px] min-w-[20px] text-right">{cpuUsage[srv.nodeIdx]}%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="md:col-span-2 glass-panel border border-card-border/60 rounded-3xl p-6 sm:p-8 flex flex-col justify-between group relative overflow-hidden">
            <div className="absolute -right-16 -top-16 w-32 h-32 rounded-full bg-accent-teal/5 blur-xl group-hover:opacity-100 transition-opacity duration-500" />
            
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 mb-8">
              <div className="text-left max-w-md">
                <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-card-border w-fit mb-4">
                  <Smartphone className="w-6 h-6 text-accent-teal" />
                </div>
                <h3 className="text-xl font-bold font-display text-white mb-2">Mobile & Web Engineering</h3>
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">
                  Building cross-platform Flutter applications published to the App Store and Google Play, alongside blazing fast web interfaces built in React and Next.js.
                </p>
              </div>

              {/* Mobile Device Simulator Dashboard visual */}
              <div className="flex-1 w-full max-w-[220px] bg-slate-950/80 rounded-3xl border-2 border-slate-800 overflow-hidden font-sans text-left relative shadow-xl mx-auto sm:mx-0 shrink-0">
                ...
                <div className="pt-7 p-3 space-y-2.5 text-[9px] text-slate-200">
                  ...
                  <div className="bg-[#0f1626] border border-card-border/60 p-2.5 rounded-xl flex items-center justify-between gap-2 shadow-inner">
                    ...
                    {/* Ring loader */}
                    <div className="w-7 h-7 rounded-full border-2 border-t-accent-teal border-slate-700 animate-spin" style={{ animationDuration: "3s" }} />
                  </div>
                  ...
                  <div className="bg-[#0f1626] border border-card-border/60 p-2 rounded-xl flex items-center justify-between">
                    <div>
                      <span className="text-slate-500 block text-[6px]">CONTRACTORSYNC</span>
