...
export default function ContactTerminal() {
  const [step, setStep] = useState<"init" | "name" | "email" | "message" | "confirm" | "sending" | "done" | "idle">("init");
  ...
  return (
    ...
    >
      ...
      <div className="p-6 h-[380px] overflow-y-auto flex flex-col gap-2 text-xs sm:text-sm select-text scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
      ...
      <div className="bg-[#0b101d]/60 border-t border-card-border px-5 py-3 flex items-center justify-between text-[10px] text-slate-500">
        ...
        <div className="flex gap-2.5 ml-auto w-full sm:w-auto justify-between sm:justify-start">
          {step === "confirm" && (
            <div className="flex gap-1.5">
              <button
                onClick={() => { setInputValue("y"); setTimeout(handleInputSubmit, 50); }}
              ...
              <button
                onClick={() => { setInputValue("n"); setTimeout(handleInputSubmit, 50); }}
